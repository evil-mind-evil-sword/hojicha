...
use ratatui::{
    layout::{Constraint, Direction, Layout as RatatuiLayout, Rect, Alignment as RatatuiAlignment},
    Frame,
    widgets::{Block, Borders},
};
use super::{Style, ColorProfile};
...
pub trait Element {
    ...
    fn with_style(self, style: Style) -> Self where Self: Sized;
}
...
pub fn join_horizontal<E: Element + 'static>(
    alignment: VAlign,
    elements: Vec<E>,
) -> LayoutBuilder {
...
pub fn join_vertical<E: Element + 'static>(
    alignment: HAlign,
    elements: Vec<E>,
) -> LayoutBuilder {
...
impl Element for StyledText {
    fn render(&self, frame: &mut Frame, area: Rect, profile: &ColorProfile) {
        ...
        let padded_area = if padding.top > 0 || padding.right > 0 || padding.bottom > 0 || padding.left > 0 {
            Rect {
